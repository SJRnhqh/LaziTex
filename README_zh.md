...
│   ├── 🌍 environment.go          # 环境检测、安装器接口定义及安装/卸载逻辑
│   ├── 🔨 build.go                # 跨平台编译工作流
│   ├── 📦 package.go              # 自动包检测与安装
...
- **分层架构**：清晰的依赖层次 - `model`（基础层）→ `config`（第一层）→ `lang`（第二层）→ `core`（业务逻辑层）
- **模块化设计**：职责分离，独立的包管理国际化（`lang/`）、配置（`config/`）和共享模型（`model/`）
- **依赖注入**：平台特定的检查器在 `main.go` 中创建并注入到核心逻辑，避免循环依赖
- **接口驱动**：`EnvironmentChecker` 接口使平台扩展变得简单
- **优先级系统**：工具按优先级分类（⭐ 核心、🔹 重要、🔸 可选）
- **类型安全**：共享数据结构（如 `Language`）定义在 `model/` 中，防止循环导入
